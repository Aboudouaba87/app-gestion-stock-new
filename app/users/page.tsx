...
export default function UsersPage() {
  ...
  const resetFilters = useCallback(() => {
    ...
    setRoleFilter("all");
    setStatusFilter("all");
    toast({ title: "Filtres réinitialisés", description: "Tous les filtres ont été effacés" });
  }, [toast]);
  ...
  const handleCreateUser = useCallback(async () => {
    if (!isCreateFormValid) {
      toast({ title: "Erreur de validation", description: "Veuillez remplir correctement le formulaire", variant: "destructive" });
      return;
    }

    const warehouseLabel = warehouses.find((w) => w.value === newUser.warehouse)?.label || "";
    const payload = {
      name: newUser.name,
      email: newUser.email,
      phone: newUser.phone,
    ...
    try {
      ...
      toast({ title: "Utilisateur créé", description: `${created.name} ajouté` });
    } catch (err: any) {
  ...
  const handleEditUser = useCallback((user: AppUser) => {
    ...
    setEditUser({
      ...
      phone: user.phone,
      role: user.role,
      warehouse: warehouses.find((w) => w.label === user.warehouse)?.value || "",
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        role: editUser.role,
        warehouse: warehouses.find((w) => w.value === editUser.warehouse)?.label || "",
      ...
      setEditingUser(null);
      toast({ title: "Utilisateur modifié", description: `${updated.name} mis à jour` });
    } catch (err: any) {
              ...
              <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="grid gap-4 py-4">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Select value={newUser.warehouse} onValueChange={(value) => setNewUser((p) => ({ ...p, warehouse: value }))}>
                      <SelectTrigger className="col-span-3">
                  ...
                  <div className="grid grid-cols-4 items-center gap-4">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(checked) => setNewUser((p) => ({ ...p, status: Boolean(checked) }))}
                      className="col-span-3"
                    />
                  </div>
                  ...
                  <div className="flex justify-end space-x-2">
                    ...
                    <Button onClick={handleCreateUser} disabled={isCreating || !isCreateFormValid} className={!isCreateFormValid ? "opacity-50 cursor-not-allowed" : ""}>
                  ...
                  <Collapsible>
                    <CollapsibleTrigger asChild>
                      <Button variant="outline" className="relative bg-transparent">
                        ...
                        Filtres
                        {activeFiltersCount > 0 && <Badge className="ml-2 bg-blue-600 text-white text-xs px-1.5 py-0.5">{activeFiltersCount}</Badge>}
                      </Button>
                    </CollapsibleTrigger>
    ...
    {isLoading ? (
      ...
      <>
        ...
        <div className="hidden md:block overflow-x-auto">
          <Table className="min-w-[900px]"> {/* min-w to keep columns readable */}
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="hidden lg:table-cell">Entrepôt</TableHead>
            ...
            <TableBody>
              {filteredUsers.map((user) => (
                <TableRow key={user.id}>
                  ...
                  <TableCell className="hidden lg:table-cell">{user.warehouse}</TableCell>

                  <TableCell className="hidden sm:table-cell">{getStatusBadge(user.status)}</TableCell>

                  <TableCell className="hidden xl:table-cell text-sm text-gray-600">{user.lastlogin ?? "Jamais"}</TableCell>

                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleEditUser(user)} aria-label={`Modifier ${user.name}`}>
                        <Edit className="h-4 w-4" />
                      </Button>

                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="ghost" size="sm" aria-label={`Supprimer ${user.name}`}>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <div className="text-sm">Êtes-vous sûr de vouloir supprimer <strong>{user.name}</strong> ?</div>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            ...
                            >
                              {isDeletingId === user.id ? "Suppression..." : "Supprimer"}
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
          ...
          {filteredUsers.map((user) => (
            <div key={user.id} className="bg-white border rounded-lg p-4 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600 truncate max-w-[220px]">{user.email}</div>
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-sm text-gray-600">
                <div className="px-2 py-1 bg-gray-100 rounded">{user.warehouse}</div>
                <div className="px-2 py-1 bg-gray-100 rounded">{user.lastlogin ?? "Jamais connecté"}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="edit-name" value={editUser.name} onChange={(e) => setEditUser((p) => ({ ...p, name: e.target.value }))} className="col-span-3" />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-email" className="text-right">Email *</Label>
              <Input id="edit-email" value={editUser.email} onChange={(e) => setEditUser((p) => ({ ...p, email: e.target.value }))} className="col-span-3" />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="edit-phone" className="text-right">Téléphone *</Label>
              <Input id="edit-phone" value={editUser.phone} onChange={(e) => setEditUser((p) => ({ ...p, phone: e.target.value }))} className="col-span-3" />
            </div>

            <div className="grid grid-cols-4 items-center gap-4">
              <Label className="text-right">Rôle *</Label>
              <Select value={editUser.role} onValueChange={(v) => setEditUser((p) => ({ ...p, role: v }))}>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select value={editUser.warehouse} onValueChange={(v) => setEditUser((p) => ({ ...p, warehouse: v }))}>
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Switch checked={editUser.status} onCheckedChange={(c) => setEditUser((p) => ({ ...p, status: Boolean(c) }))} className="col-span-3" />
            </div>

            <div className="flex justify-end space-x-2">
              <Button variant="outline" onClick={() => setIsEditModalOpen(false)}>Annuler</Button>
              <Button onClick={handleSaveEdit} disabled={isSaving || !isEditFormValid}>{isSaving ? "Sauvegarde..." : "Sauvegarder"}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
